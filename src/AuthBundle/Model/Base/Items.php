...
abstract class Items implements ActiveRecordInterface
{
     ...
     * TableMap class name
     */
    const TABLE_MAP = '\\AuthBundle\\Model\\Map\\ItemsTableMap';
    ...
    /**
     * Initializes internal state of AuthBundle\Model\Base\Items object.
     */
     ...
     * @return $this|\AuthBundle\Model\Items The current object (for fluent API support)
     */
    public function setId($v)
    {
        ...
        if ($this->id !== $v) {
            $this->id = $v;
            $this->modifiedColumns[ItemsTableMap::COL_ID] = true;
        }
    ...
    public function hydrate($row, $startcol = 0, $rehydrate = false, $indexType = TableMap::TYPE_NUM)
    {
        try {
            ...
            $col = $row[TableMap::TYPE_NUM == $indexType ? 0 + $startcol : ItemsTableMap::translateFieldName('Id', TableMap::TYPE_PHPNAME, $indexType)];
            $this->id = (null !== $col) ? (int) $col : null;

            $col = $row[TableMap::TYPE_NUM == $indexType ? 1 + $startcol : ItemsTableMap::translateFieldName('ItemName', TableMap::TYPE_PHPNAME, $indexType)];
            ...
            return $startcol + 2; // 2 = ItemsTableMap::NUM_HYDRATE_COLUMNS.

        } catch (Exception $e) {
            throw new PropelException(sprintf('Error populating %s object', '\\AuthBundle\\Model\\Items'), 0, $e);
        }
    }
        ...
        if (!$this->alreadyInSave) {
            ...
            if ($this->postssScheduledForDeletion !== null) {
                if (!$this->postssScheduledForDeletion->isEmpty()) {
                    foreach ($this->postssScheduledForDeletion as $posts) {
    ...
    protected function doInsert(ConnectionInterface $con)
    {
        ...
        $this->modifiedColumns[ItemsTableMap::COL_ID] = true;
        if (null !== $this->id) {
            throw new PropelException('Cannot insert a value for auto-increment primary key (' . ItemsTableMap::COL_ID . ')');
        }

         // check the columns in natural order for more readable SQL queries
        if ($this->isColumnModified(ItemsTableMap::COL_ID)) {
     ...
     *                TableMap::TYPE_COLNAME, TableMap::TYPE_FIELDNAME, TableMap::TYPE_NUM.
     *                Defaults to TableMap::TYPE_PHPNAME.
     * @return $this|\AuthBundle\Model\Items
     */
     ...
     * @param string $keyType The type of keys the array uses.
     *
     * @return $this|\AuthBundle\Model\Items The current object, for fluid interface
     */
    ...
    public function buildCriteria()
    {
        ...
        if ($this->isColumnModified(ItemsTableMap::COL_ID)) {
            $criteria->add(ItemsTableMap::COL_ID, $this->id);
        }
        if ($this->isColumnModified(ItemsTableMap::COL_ITEM_NAME)) {
     ...
     * objects.
     *
     * @param      object $copyObj An object of \AuthBundle\Model\Items (or compatible) type.
     ...
     *
     * @param  boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
     * @return \AuthBundle\Model\Items Clone of current object.
     * @throws PropelException
     */
    ...
    public function initPostss($overrideExisting = true)
    {
        ...
        $this->collPostss->setModel('\AuthBundle\Model\Posts');
    }
    ...
    public function removePosts(ChildPosts $posts)
    {
        if ($this->getPostss()->contains($posts)) {
            ...
            $this->postssScheduledForDeletion[]= $posts;
            $posts->setItems(null);
        }
